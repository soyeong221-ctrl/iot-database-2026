...
- ERD 모델링 툴
    ...
    - Draw.io: 개념 논리 ERD 작성 가능, 기능 미흡 (무료)
        - https://app.diagrams.net/
    - `erdcloud` - 한국에서 개발한 웹기반 모델링툴. 논리/물리 ERD 작성, 내보내기 기능 (무료/유료)
...
- 속성(Attribute)
    - 개체가 가지는 성질
    - 일반 속성(타원), 키 속성(글자에 밑줄), 다중 속성(이중타원), 유도속성(점선타원), ...
...
- 여기까지 개념 ER모델이지만, 현재는 논리 ER모델과 통합해서 작성하고 있음, IE 표기법
...
- 수강관리 시스템화 되기 이전 문제점
    - 이상현상
        - 삽입 이상: 수강생이 없는 새 과목은 추가가 어려움(과목, 강사명, 강사전화만 입력하면 의미있는 데이터가 아님)
        - 수정 이상: 박지은 강사의 전화번호가 바뀌면 Python 과목을 듣는 모든 행을 찾아서 수정 필요
        - 삭제 이상: 특정 학생의 과목신청 내용을 삭제하면 과목정보, 강사정보 모두 사라질 수 있음

- 정규화
    - 이상현상, 종속성문제, 이행성문제 등을 제거하는 작업

- **제1정규형**(1NF)
    - `도메인(속성값)이 원자값`이어야 함. 한 컬럼에 여러 값이 들어가면 안 됨
    - 전화번호 컬럼에 010-2222-2222, 010-3333-3333 두 개의 값이 들어가면 안 됨
...
- **제2정규형**(2NF)
    - `부분적 (함수) 종속 제거`: 기본키의 일부에만 종속되는 컬럼은 제거함
    - 학생명, 전화번호 속성은 학번에만 종속 -> 학생정보 분리
...
- MYSQL 데이터베이스, 사용자 직접 생성
    ...
    - 강사정보(Lecture) 테이블의 Ins_No가 NULL과 NOT NULL 사이의 관계 비교
    - 왼쪽은 강사정보가 없어도 입력 가능, 오른쪽은 필수로 강사번호 입력
...
- **BCNF(Boyce-Codd 정규형)**
    ...
    - (학생, 과목) -> 강사 결정가능
    - 과목만 가지고 강사 결정가능, 학생은 강사 지정할 키가 아님
...
- 순번 처리용 테이블 nums
- 100만건씩 insert용 저장프로시저 생성 insert_big_orders - [쿼리](./day08/4.대량데이터_저장프로시저.sql)
...
- 위 내용 분석: 제일 아래 -> 부터 분석
    ...
    3. Sort: order_date 내림차순 정렬, actual time 2568..2568이 최종 처리시간

- 인덱스 추가 후

```text
-> Index lookup on orders_big using idx_orders_customer_id_and_order_date (customer_id=123456) (reverse)  (cost=27 rows=26) (actual time=0.461..1.8 rows=26 loops=1)
```
...
- MySQL C API 함수목록
    ...
    - mysql_affected_rows() - 쿼리실행 처리 행수 리턴
    - mysql_close() - DB연결 종료
...
- MySQL Connector/C++
    ...
    - 객체화, 예외처리 기능 고급화
    - 운영체제 환경 영향 지대
    - 설정난이도 높음 + Visual Studio 설정 까다로움
    - 유지보수 구조적으로는 좋음
